...
impl TaskManager {
    ...
    fn run_first_task(&self) -> ! {
        ...
        task0.task_start_time = get_time();
    ...
    fn run_next_task(&self) {
        if let Some(next) = self.find_next_task() {
            ...
            // update start time if it never been used
            if inner.tasks[next].task_start_time == 0 {
                inner.tasks[next].task_start_time = get_time();
            }
    ...
    fn get_current_task_info(&self, task_info: &mut TaskInfo) {
        ...
        task_info.status = tasks[current].task_status;
        task_info.syscall_times = tasks[current].task_syscall_times;
        task_info.time = time_to_ms(get_time() - tasks[current].task_start_time);
    }

    // update syscall times
    fn update_current_task_syscall(&self, syscall_id: usize) {
        let mut inner = self.inner.exclusive_access();
        let current = inner.current_task;
        let tasks = &mut inner.tasks;
        tasks[current].task_syscall_times[syscall_id] += 1;
    }
}
